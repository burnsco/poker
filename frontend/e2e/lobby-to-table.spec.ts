...
test("shows open games, enters the bot table, and starts the hand", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(page.locator(".lobby-card").filter({ hasText: "Bot Warmup Table" })).toBeVisible();
  await expect(page.locator(".lobby-card").filter({ hasText: "Blinds 10 / 20" })).toBeVisible();
  ...
  await expect(
    page.locator(".backend-grid div").filter({ has: page.locator("span", { hasText: "Table" }) }),
  ...
  await expect(page.getByRole("button", { name: "Hide Hand Log" })).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: /Sit Out|Sit In/ })).toBeVisible({
    timeout: 30_000,
  });
