...
function LobbyScreen() {
  ...
  const openAuthMode = useCallback(
    (mode: "login" | "register", notice?: LobbyNotice) => {
      console.log("App: openAuthMode called with mode:", mode);
      setAuthMode(mode);
  ...
  }, [authMode, authEmail, authName, authPassword, clearAuthError, login, register]);
  ...
  return (
    <div id="app" className="app-lobby dark">
          ...
          {loading ? (
            ...
            <Dialog open={showAuthModal} onOpenChange={setShowAuthModal}>
              ...
              <DialogContent className="sm:max-w-106.25">
                ...
                {authMessage && <p className="text-sm text-destructive">{authMessage}</p>}
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    void submitAuth();
                ...
                >
                  ...
                  <div className="flex justify-between">
                    <Button type="submit" disabled={authSubmitDisabled}>
                      {authPending
                        ? authMode === "login"
                          ? "Logging in..."
                          : "Creating..."
      ...
      <main className="lobby-shell glass-panel anim-slide-up" style={{ animationDelay: "0.08s" }}>
        <section className="lobby-hero">
          <div className="lobby-copy">
            ...
            <p className="lobby-subcopy">
              Track active games, jump into open seats, and spin up a private table in one click.
            </p>
          </div>
          <div className="lobby-highlights">
            ...
            <div>
              ...
              <strong>{games.reduce((sum, game) => sum + game.humanPlayers, 0)}</strong>
            </div>
            <div>
          ...
          <div className="lobby-column wide">
            ...
            <div className="extras-grid">
              <div className="extra-card glass-panel">
                ...
                <p>6-max turbo cash with deep stacks and faster blind pressure.</p>
              </div>
              <div className="extra-card glass-panel">
                <h3>Daily Challenge</h3>
                <p>Win one showdown with pocket pairs and log three clean folds.</p>
              </div>
              <div className="extra-card glass-panel">
                <h3>Table Notes</h3>
                <p>
                  Custom tables can be started empty and filled with bots from inside the table
                  view.
                </p>
              </div>
            </div>
...
function TableScreen({ tableId, isCustomTable }: { tableId: string; isCustomTable: boolean }) {
  ...
  const canAddBot = backendTable?.hand_state.status !== "in_progress" && emptySeatCount > 0;
  ...
  const sendActionWithDebug = useCallback(
    async (action: string, payload?: TableActionPayload, source = "ui") => {
      ...
      logControlDebug(`${source}: sending action=${action} payload=${payloadLabel}`);
      ...
      try {
        ...
        const reason = error instanceof Error ? error.message : "Unknown action failure";
  ...
  const handleSliderPreset = useCallback(
    (preset: "min" | "halfpot" | "pot" | "max") => {
      ...
      if (preset === "halfpot") {
        ...
        logControlDebug(`preset:halfpot -> ${value} (pot=${potVal} min=${min} max=${max})`);
        return;
      }
      ...
      if (preset === "pot") {
        ...
        setSliderValue(value);
        logControlDebug(`preset:pot -> ${value} (pot=${potVal} min=${min} max=${max})`);
        return;
      }
  ...
  const runPlayerAction = useCallback(
    async (action: "fold" | "check" | "call" | "bet" | "raise", amount?: number) => {
  ...
  const handleFoldClick = useCallback(() => {
    ...
    logControlDebug(`click:fold disabled=${String(foldBtn?.disabled ?? false)}`);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const game = new PokerGameClient(playerId, (action, payload) =>
  ...
  useEffect(() => {
    ...
    const actionState = document.getElementById("action-state")?.textContent?.trim() || "n/a";
    logControlDebug(
      [
        `state:update`,
    ...
    <div id="app" className="app-table dark">
      ...
      {!user && !loading ? (
        <div className="table-auth-banner glass-panel">
          ...
          <button type="button" className="btn primary tiny" onClick={navigateToLobby}>
            Log In from Lobby
          </button>
        </div>
      ) : null}
      {tableNotice ? (
        <div className="table-inline-notice glass-panel">
          ...
          <button type="button" className="btn tiny" onClick={() => setTableNotice(null)}>
            Dismiss
          </button>
        </div>
      ) : null}
          ...
          <div className="table-seats">
            {Array.from({ length: MAX_SEATS }, (_, seat) => {
              ...
              const queuedSeatClaim =
                ...
                (backendPlayer.stack > 0 || backendTable?.hand_state.status === "in_progress");
              const canClaimSeat =
                user != null &&
                !ownedPlayer &&
                !pendingPlayer &&
                  ...
                  <div className="seat-anchor" style={style}>
                    ...
                    >
                      ...
                      {canClaimSeat ? (
                        ...
                        <button type="button" className="seat-claim-btn" onClick={navigateToLobby}>
                          Login for Seat {backendSeat}
                        </button>
                      ) : reservedSeatLabel ? (
                        <div className="seat-reserved-badge">{reservedSeatLabel}</div>
                      ) : null}
                    </div>
                  </div>

                  <div className="seat-bet-anchor" style={style}>
                    <div className={`seat-bet ${posClass}`} id={`${playerId}-bet`}></div>
                  </div>
                </div>
              );
            })}
          ...
          <div className="controls-strip-actions">
            {!user ? (
              ...
              <>
                ...
                >
                  {pendingPlayer.will_play_next_hand ? "Sit Out" : "Play Next Hand"}
                </button>
              </>
            ) : (
            ...
            {isCustomTable && user && backendTable?.hand_state.status !== "in_progress" ? (
              <>
                <button
                  type="button"
                  className="btn tiny"
                  disabled={!canAddBot}
                  onClick={() => void sendActionWithDebug("add_bot", undefined, "table-setup")}
                >
                  + Bot
                </button>
                <button
                  ...
                  onClick={() => void sendActionWithDebug("clear_table", undefined, "table-setup")}
                >
                  Clear
                </button>
              </>
            ) : null}
            {backendTable?.game_state === "waiting_for_hand" && user && readySeats >= 2 ? (
              <button
                type="button"
                className="btn primary tiny"
                onClick={() => void sendActionWithDebug("next_hand", undefined, "manual")}
              >
                Next Hand
              </button>
            ) : null}
      ...
      <div className="bottom-right-dock">
        ...
        >
          <div className="backend-status-header">
            {!backendOverlayCollapsed ? <p className="backend-eyebrow">Dev stats</p> : null}
            <div className="backend-status-title-row">
              {!backendOverlayCollapsed ? <h2>{backendState}</h2> : null}
              <button
                type="button"
                className="btn tiny backend-toggle"
                onClick={() => setBackendOverlayCollapsed((collapsed) => !collapsed)}
              >
          ...
          <div className="backend-grid">
            ...
            <div>
              <span>Framework</span>
              <strong>{backendHealth?.framework || "system"}</strong>
            </div>
            <div>
              <span>Table</span>
