...
export class Renderer {
  ...
  update() {
    ...
    // Derive heroSeatIndex from the already-snapshotted players array so
    // there is no chance of a second buildState() call returning a different
    // heroSeatIndex than the players we are about to render against.
    const heroSeatIndex = players.findIndex((p) => p?.isCurrentUser === true);
    const heroPlayer = heroSeatIndex !== -1 ? players[heroSeatIndex] : null;
    const heroActive =
      heroSeatIndex !== -1 &&
      s.actionTo === heroSeatIndex &&
      !showAllCards &&
      heroPlayer != null &&
      (heroPlayer.status === "ACTIVE" || heroPlayer.status === "READY");
    ...
    if (s.winners !== null) {
      ...
      // Never resolve the actor name to the hero's own name — if it's our
      // seat that's acting but heroActive somehow evaluated false (e.g. a
      // brief status mismatch during state transition), show "Your turn"
      // rather than the confusing "[YourName] is acting..." message.
      if (actorSeat !== null && actorSeat === heroSeatIndex) {
        actionStateEl.textContent = "Your turn.";
      } else {
        const actorName =
