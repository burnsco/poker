...
import type { PokerGame } from "../game/PokerGame";
...
export class Renderer {
	...
	private updateReviewSidebar() {
		...
		const nextHandBtn = document.getElementById(
			"btn-next-hand",
	...
	private updatePlayerArea(
		...
		if (!player) {
			...
			cardsEl.innerHTML =
				'<div class="card-slot hole-slot"></div><div class="card-slot hole-slot"></div>';
	...
	update() {
		...
		const heroActive =
			s.actionTo === 0 && !isShowdown && p1 && p1.status === "ACTIVE";
		...
		if (s.winners !== null) {
			actionStateEl.textContent =
				"Hand complete. Review the sidebar and press Next Hand when ready.";
		} else if (isShowdown) {
