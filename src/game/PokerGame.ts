...
export type PokerSoundEvent =
	...
	| { type: "uncalled_bet_returned"; seat: number; amount: number }
...
export class PokerGame {
	...
	constructor(config?: Partial<GameConfig>) {
		...
		this.engine.on((rawAction, oldState, newState) => {
			...
			const soundEvent = this.soundEventFromAction(
				action,
				newState as GameState,
			);
			if (soundEvent) {
				this.emitSoundEvent(soundEvent);
			}
			const actionLine = this.describeAction(
				action,
			...
			if (action.type === "DEAL") {
				this.describeForcedBets(newState as GameState).forEach((line) => {
					this.appendActionLog(line);
				});
			}

			if (
				oldState.street !== newState.street &&
				newState.street !== "PREFLOP"
			) {
				const street = (newState as GameState).street;
				if (street === "FLOP") this.emitSoundEvent({ type: "street_flop" });
				if (street === "TURN") this.emitSoundEvent({ type: "street_turn" });
				if (street === "RIVER") this.emitSoundEvent({ type: "street_river" });
				if (street === "SHOWDOWN")
					this.emitSoundEvent({ type: "street_showdown" });

				const streetMessage = this.describeStreetTransition(
					oldState as GameState,
					newState as GameState,
				);
			...
			if (newState.winners && !oldState.winners) {
				this.emitSoundEvent({
					type: "pot_awarded",
					winners: newState.winners.length,
				});
				this.clearTimer();
				this.clearAIDelay();
				...
				this.appendActionLog(
					"Hand complete. Review the hand log, then press Next Hand.",
				);
	...
	get stage(): string {
		return this.engine.state.street
			? this.engine.state.street.toLowerCase()
			: "waiting";
	}
	...
	private dispatchAction(action: {
		...
		this.engine.act(action as Action);
	}

	private emitSoundEvent(event: PokerSoundEvent) {
		this.onSoundEvent(event);
	}

	private seatByPlayerId(state: GameState, playerId?: string): number | null {
	...
	private soundEventFromAction(
		action: Action,
		newState: GameState,
	): PokerSoundEvent | null {
		const playerId = "playerId" in action ? action.playerId : undefined;
		const seat = this.seatByPlayerId(newState, playerId);
		if (action.type === "DEAL") return { type: "hand_start" };
		if (action.type === "FOLD" && seat !== null) return { type: "fold", seat };
		if (action.type === "CHECK" && seat !== null)
			return { type: "check", seat };
		if (action.type === "CALL" && seat !== null) return { type: "call", seat };
		if (action.type === "BET" && seat !== null) {
			return { type: "bet", seat, amount: action.amount };
		}
		if (action.type === "RAISE" && seat !== null) {
			return { type: "raise", seat, amount: action.amount };
		}
		if (action.type === "SHOW" && seat !== null) return { type: "show", seat };
		if (action.type === "MUCK" && seat !== null) return { type: "muck", seat };
		if (action.type === "TIMEOUT" && seat !== null) {
			return { type: "timeout", seat };
		}
		if (action.type === "UNCALLED_BET_RETURNED" && seat !== null) {
			return { type: "uncalled_bet_returned", seat, amount: action.amount };
		}
		if (action.type === "ADD_CHIPS" && seat !== null) {
			return { type: "add_chips", seat, amount: action.amount };
		}
		return null;
	}
	...
	private describeStreetTransition(
		oldState: GameState,
		newState: GameState,
	): string {
	...
	private describeAction(
		...
		if (action.type === "CALL") {
			...
			const amount = Math.max(
				0,
				(newPlayer?.totalInvestedThisHand || 0) -
					(oldPlayer?.totalInvestedThisHand || 0),
			);
			return amount > 0
				? `${actorName} called ${amount}.`
				: `${actorName} called.`;
		}
		if (action.type === "BET") {
	...
	private buildHandResult(state: GameState): HandResultSummary {
		...
		const winnerNames = winners
			.map(
				(winner) =>
					state.players[winner.seat]?.name || `Seat ${winner.seat + 1}`,
			)
			.join(", ");
		...
		for (const winner of winners) {
			...
			const winnerRank =
				winner.handRank || this.evaluateRank(winnerPlayer, state.board);
			const winnerCards = winner.hand || winnerPlayer?.hand || null;
			const rankText = winnerRank ? ` with ${winnerRank}` : "";
			const cardsText = winnerCards
				? ` (${this.formatCards(winnerCards)})`
				: "";
		...
		if (showdownEvaluations.length > 0) {
			...
			showdownEvaluations.forEach((entry) => {
				...
				lines.push(
					`${entry.name}: ${rankText} (${this.formatCards(entry.cards)}).`,
				);
			});

			const topWinner = showdownEvaluations.find((entry) =>
				winnerSeatSet.has(entry.seat),
			);
			if (topWinner) {
				showdownEvaluations
		...
		const topWinnerPlayer = topWinner
			? this.getPlayerByWinner(state, topWinner)
			: null;
		const topWinnerName =
			topWinnerPlayer?.name ||
			(topWinner ? `Seat ${topWinner.seat + 1}` : "winner");
		const topWinnerRank =
			topWinner?.handRank || this.evaluateRank(topWinnerPlayer, state.board);
		...
		if (heroRank && topWinnerRank) {
			...
			const winnerCardsText = topWinnerCards
				? ` (${this.formatCards(topWinnerCards)})`
				: "";
			lines.push(
				`You lost with ${heroRank}${heroCardsText} against ${topWinnerName}'s ${topWinnerRank}${winnerCardsText}.`,
			);
		} else {
	...
	private handleTimeout(seatIndex: number) {
		...
		try {
			...
			this.dispatchAction({ type: "CHECK", playerId: p.id });
		} catch {
			this.dispatchAction({ type: "FOLD", playerId: p.id });
		}
		this.triggerAILogic();
	}
		...
		if (this.config.bettingStructure === "POT_LIMIT") {
			...
			if (isRaise && currentBet > 0) {
				...
				if (targetAmount > maxRaiseTarget) {
					this.onMessage(
						`Pot-limit exceeded. Max raise target is ${maxRaiseTarget}.`,
					);
					return false;
				}
			}
	...
	handlePlayerAction(
		action: "fold" | "call" | "raise" | "bet" | "check" | "straddle",
		amount?: number,
	) {
		...
		try {
			if (action === "fold") {
				this.dispatchAction({ type: "FOLD", playerId: "p1" });
				this.onMessage("You folded.");
			} else if (action === "check") {
				this.dispatchAction({ type: "CHECK", playerId: "p1" });
				this.onMessage("You checked.");
			} else if (action === "call") {
				this.dispatchAction({ type: "CALL", playerId: "p1" });
				...
				const playerBet = this.getPlayerBetThisStreet(
					this.engine.state,
					p1.seat,
				);
				const minimumRaiseTarget = this.getMinimumRaiseTarget(
					this.engine.state,
				);
				const maxTarget = playerBet + p1.stack;
				const requestedTarget =
					amount !== undefined ? amount : minimumRaiseTarget;
				...
				const maxTarget = playerBet + p1.stack;
				const minimumOpenBet = Math.min(
					maxTarget,
					Math.max(1, this.engine.state.bigBlind),
				);
				...
				if (
					!this.config.allowStraddle ||
					this.engine.state.street !== "PREFLOP"
				) {
	...
	triggerAILogic() {
		...
		this.aiDelayTimer = setTimeout(() => {
			...
			if (aiPlayer.status !== "ACTIVE" || aiPlayer.stack <= 0) {
				try {
					this.dispatchAction({ type: "TIMEOUT", playerId: aiPlayer.id });
				} catch {
			...
			try {
				if (toCall === 0) {
					...
					if (rand > 0.7 && handVal >= 2) {
						...
						this.dispatchAction({ type: "CHECK", playerId: aiPlayer.id });
					...
					const raiseTo =
						this.config.bettingStructure === "FIXED_LIMIT"
							? currentBet +
								(currentS.street === "PREFLOP" || currentS.street === "FLOP"
									? this.config.smallBlind
									: this.config.bigBlind)
							: Math.min(
									playerBet + aiPlayer.stack,
									this.getMinimumRaiseTarget(currentS),
								);
					...
					if (this.validateBetOrRaise(aiPlayer.id, raiseTo, true)) {
						...
						// Fallback call
						this.dispatchAction({ type: "CALL", playerId: aiPlayer.id });
					...
					this.dispatchAction({ type: "FOLD", playerId: aiPlayer.id });
