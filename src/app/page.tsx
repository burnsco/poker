...
import { PokerSoundEngine } from "../ui/PokerSoundEngine";
...
export default function Home() {
	...
	useEffect(() => {
		if (typeof window !== "undefined" && !gameRef.current) {
			...
			if (!slider || !display || !callBtn || !raiseBtn || !nextHandBtn) {
				sounds.dispose();
				return;
			}
			...
			const onCall = () => {
				const action = (callBtn.getAttribute("data-action") || "call") as
					| "call"
					| "check";
				game.handlePlayerAction(action);
			};
			const onRaise = () => {
				const action = (raiseBtn.getAttribute("data-action") || "raise") as
					| "raise"
					| "bet";
			...
			return () => {
				...
				document
					...
					?.removeEventListener("click", onHalf);
				...
				document
					...
					?.removeEventListener("click", onFold);
	...
	return (
		<div id="app">
			...
			<div className="table-layout">
							...
							return (
								<div className="seat-slot" key={playerId}>
									<div className="seat-anchor" style={style}>
										<div
											className={`player-area${seat === 0 ? " is-you" : ""}`}
											id={`seat-${seat}`}
										>
											<div className="avatar-row">
												...
												{seat === 0 && (
													<div
														className="player-cards compact"
														id={`${playerId}-cards`}
													>
				...
				<aside
					className="action-sidebar glass-panel anim-slide-up"
			...
			>
				...
				<div className="btn-group">
					<button type="button" className="btn tiny" id="btn-min">
						Min
					</button>
					<button type="button" className="btn tiny" id="btn-half">
						1/2
					</button>
					<button type="button" className="btn tiny" id="btn-pot">
						Pot
					</button>
					<button type="button" className="btn tiny" id="btn-max">
						Max
					</button>
				</div>

				<div className="main-actions">
					<button type="button" className="btn danger" id="btn-fold">
						Fold
					</button>
					<button
						type="button"
