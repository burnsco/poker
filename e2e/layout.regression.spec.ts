...
for (const mode of modes) {
	test(`keeps seats and controls in bounds (${mode.name})`, async ({
		page,
	}) => {
		...
		expect(
			...
			"hero seat should stay above controls",
		).toBeLessThanOrEqual(controlsRect.y + 8);
		...
		for (let seat = 0; seat < 8; seat += 1) {
			...
			expect(
				ellipseDistance,
				`seat-${seat} should sit near the table oval`,
			).toBeGreaterThan(minEllipseDistance);
			expect(
				ellipseDistance,
				`seat-${seat} should not drift off the oval`,
			).toBeLessThan(0.99);
		}
	});
}
